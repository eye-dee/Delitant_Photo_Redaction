...
#include <stdio.h>       // s//printf(..), fopen(..)
#include <stdarg.h>		 // So we can use ... (in //printf)
...
/*
__force void //printf(const char *fmt, ...) 
{
    va_list parms;
    char buf[256];
...
void WriteBMP24(const char* szBmpFileName, int Width, int Height, color* RGB)
{
    ...
    for (int y=Height-1; y>=0; y--)
    {
        ...
        {
            int i = (x + (Width)*y);
			unsigned int rgbpix = (RGB[i].r << 16)|(RGB[i].g << 8)|(RGB[i].b << 0);
            fwrite(&rgbpix, 3, 1, fp);
        }
        if (iNumPaddedBytes>0)
        {
